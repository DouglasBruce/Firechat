...
@Pipe({
  name: 'lastUpdatedConvertor'
})
export class lastUpdatedConvertorPipe implements PipeTransform {
    ...
    transform(milli: number, arg1: any): string {
        ...
        let hoursPassed = Math.floor(milliPassed / 3600000);
        ...
        if(daysPassed == 0) {
            ...
            if(daysPassed > 0 && daysPassed <= 365) {
    ...
    generateShortDate(d: string | number, m: number) {
        return this.dateService.getMonthShortName(m) + ' ' + d;
    }

    generateLongDate(d: string | number, m: number, y: string | number) {
        return d + ' ' + this.dateService.getMonthShortName(m) + ' ' + y;
    }
